...
async function generateUser(req, res) {
  try {
    ...
    if (isCreated) {
      ...
      await axios.post(process.env.sendEmail, { email: email, name: name, password: user.password, eventName: eventName }).then((response) => { console.log(`Mail Sent to ${email} with status: `+response.status); }, (error) => { console.log("Error while mail sending"); });
